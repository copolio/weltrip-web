...
def s_detail(request):
    # # 일정표에 추가 요청시
    # if 'add_request_user' in request.POST and reuest.POST['add_request_user']:
    #     add_contentId = requst.POST.get('contentId')
    #     add_user = reqeust.POST.get('add_request_user')
    ...
    if 'contentId' in request.POST and request.POST['contentId']:
        ...
        ClickDetail(contentId = contentId, contentName = title_t, userId = user_t, userType = dis_t, cat1 = catdic.get('cat1'), cat2 = catdic.get('cat2'), cat3 = catdic.get('cat3'), date=datetime.datetime.now()).save()
